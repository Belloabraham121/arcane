...
const REGIONS = [
  { name: "JSON_API_AGENT", status: "ACTIVE", latency: "0.03 STT" },
  { name: "LLM_INFERENCE_AGENT", status: "ACTIVE", latency: "0.07 STT" },
  { name: "PARSE_WEBSITE_AGENT", status: "ACTIVE", latency: "0.10 STT" },
  { name: "REPUTATION_SETTLER", status: "ACTIVE", latency: "Live" },
]
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between border-b-2 border-foreground px-4 py-2">
        <span className="text-[10px] tracking-widest text-muted-foreground uppercase">
          somnia_agents.status
        </span>
        <span className="text-[10px] tracking-widest text-muted-foreground">
          {`BLOCK:${String(tick).padStart(4, "0")}`}
        </span>
      </div>
      <div className="flex-1 flex flex-col p-4 gap-0">
        {/* Table header */}
        <div className="grid grid-cols-3 gap-2 border-b border-border pb-2 mb-2">
          <span className="text-[9px] tracking-[0.15em] uppercase text-muted-foreground">Agent Type</span>
          <span className="text-[9px] tracking-[0.15em] uppercase text-muted-foreground">Status</span>
          <span className="text-[9px] tracking-[0.15em] uppercase text-muted-foreground text-right">Cost</span>
        </div>
        {REGIONS.map((region) => (
          <div
            key={region.name}
        ...
        <div className="mt-auto pt-4">
          ...
          <div className="h-2 w-full border border-foreground">
            <div className="h-full bg-[#ea580c]" style={{ width: "94%" }} />
          </div>
        </div>
      </div>
    </div>
